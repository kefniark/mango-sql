...
func getColumnType(column *SQLColumn) string {
	src := postgresType
	if column.Nullable {
		src = postgresNullableType
	}
...
func isIdGenerated(table *SQLTable) bool {
	...
	for _, col := range table.Columns {
		...
		filters = append(filters,
			SelectFilter{
				...
				Sql:     fmt.Sprintf(`.OrderBy("%s.%s ASC")`, table.Name, col.Name),
				Args:    []string{},
			},
			SelectFilter{
				Model:   fmt.Sprintf("%s%s", table.NameNormalized, col.NameNormalized),
